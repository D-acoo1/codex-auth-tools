...
- Clicking the reset-credit count opens a centered bubble with up to five rows. Each row shows only the expiration in the Mac's current time zone as `M.d HH:mm`; clicking elsewhere dismisses the bubble.
